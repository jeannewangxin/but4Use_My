	...
	public void run(IAction action) {
		...
		try {
			...
			if (output != null) {
				...
				// path of this script
				String scriptPath = ShowResultInGraphs.class.getProtectionDomain().getCodeSource().getLocation()
						.getPath();
				...
				try {
					if (os.contains("win")) {
						...
						command = scriptPath + "GraphGenerator/windows/pythonScript_dynamicGraphs.exe "
								+ target.getAbsolutePath();
					} else if (os.contains("mac")) {
						command = "/Library/Frameworks/Python.framework/Versions/3.7/bin/python3 " + scriptPath
								+ "GraphGenerator/mac/pythonScript_dynamicGraphs.py " + target.getAbsolutePath();
					} else if (os.contains("nix") || os.contains("nux")) {
						command = scriptPath + "GraphGenerator/unix/pythonScript_dynamicGraphs "
								+ target.getAbsolutePath();
					}
					...
					System.out.println("command: " + command);
					command = "python3 " + scriptPath + "GraphGenerator/pythonScript_dynamicGraphs.py "
							+ target.getAbsolutePath();
