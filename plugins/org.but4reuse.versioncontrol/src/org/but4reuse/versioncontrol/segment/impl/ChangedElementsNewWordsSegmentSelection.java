 ...
 * and the number of new words
 *
 * @author aarkoub
 *
 */
public class ChangedElementsNewWordsSegmentSelection implements ISegmentSelectionStrategy {
	...
	public List<Segment> getSegments(IVersionControlClient client, IVersionControlCommit startPointCommit,
		...
		inputDialog = new GenericInputSelectionDialog(currentShell, "Max number of new words",
				"Please, enter the maximum number of new words", "5");
		...
		try {
			...
			// changed elements and the max number of new words
			while (i < nbCommits - 1) {
				...
				if (nbChanges >= maxNumberChanges || nbNewWords >= maxNewWords) {
	...
	public String toString() {
		return "Number of changed elements and number of new words";
	}
